...
description: "A collection and rectification of various Lua misconceptions"
date: 2025-04-12
tags:
- lua
- programming
- performance
aliases:
- "2025-04-12-lua-misconceptions/"
- "../2025/04/12/lua-misconceptions.html"
---

This is a collection of various Lua misconceptions I have seen over the years.

To clarify matters regarding language semantics, we refer to the manual guarantee;
the relevant guarantees haven't changed from version 5.1 through 5.4.

Unfortunately the reference manual mostly refrains from statements concerning performance,
...
the reference implementation, [PUC Lua](https://www.lua.org/),
...
if you're interested in the error of `b` relative to `a`,
you might want to go with something like `math.abs((a - b) / a) <= epsilon` -- be careful with `a` close to zero though.

### "Number to string conversion is exact"
...
On LuaJIT, and on PUC Lua up to and including version 5.4, `tostring` and everything that uses it (`print`, `table.concat` etc.)
will *lose precision* of floats in order to present the user with a "nicer", rounded value
rather than requiring the programmer to explicitly demand rounding.
It is a common issue that serializers simply use `tostring`,
unnecessarily losing float precision.
This results in bugs such as large enough user IDs not surviving JSON roundtrips. [^tostring]
...
As far as the reference manual is concerned, note that `tostring` guarantees no particular formatting;
you must use formatting specifiers if you rely on a particular format,
...
However, this may result in overly verbose representations, e.g. `0.3` becomes `0.29999999999999999`, which is undesirable.
PUC Lua 5.5 changes the behavior of `tostring` to write as many significant digits
as are necessary for the number to be read back correctly, but no more.
So `tostring(0.3)` is `"0.3"`, but `tostring(0.1 + 0.2)` is `"0.30000000000000004"`.
...
Lua tables *have no order*. They are, conceptually speaking, just *associative arrays*.
They get their "order" from how you decide to iterate them.
If you decide to use `ipairs` to traverse a table, `ipairs` is doing the work of incrementing a counter to induce an order.
There need not be anything in the table that induces this order.
Your table can be all jumbled in memory. Some keys might live in the hash part, some in the array part.
...
You can -- this is guaranteed to work in Lua (but not in other languages).

Implementation-wise, the underlying hash tables usually don't delete entries, they just mark them for deletion;
...
it is wrong to conclude that they need to be implemented as just hash tables.
...
### "Setting elements to `nil` frees up space"

With Lua being a garbage-collected language, you might expect that setting table entries to `nil`
allows Lua to reclaim space. And indeed, if the key and/or value then eventually become garbage, they can be collected.
Similarly, you would expect that the *slot*, the space allocated in the table for the key-value-pair,
is freed up during garbage collection as well.
You might naively assume that garbage collection ensures that
the memory usage is roughly proportional to the size of the live data structures.
Indeed, this varies between language specifications and implementations.

<!--
And indeed this is to some extent the case in some language implementations based on brief testing:

* CPython shrinks lists (but not dictionaries);
* Node.js shrinks objects (but not arrays);
* Go shrinks maps and seemingly, at least if you only remove items at the end, slices (though this is not guaranteed)
-->

LuaJIT and PUC Lua never shrink tables when you're just setting entries to `nil`: They just mark them as free.

In the hash part, a free slot may be used again by any other key later on;
in the array part, it will only be used again for the same key.

Indeed, most straightforward Lua implementations will not reclaim slots,
because removing key-value pairs during iteration must be guaranteed to work;
a rehash would jumble the order of keys in the hash table.
...
Slots `1`, `2`, ..., `2^20 - 1` will still be taking up memory as long as `t` lives.
For the unsuspecting programmer, this behavior might easily result in memory leaks.

On the flip side, this means that you can typically set slots to `nil` to allocate space for them.
For example, to allocate an "array" with $n$ slots, you could do
...
which, at all times, will use the array part of the table.
(This trick is however obsolete when using LuaJIT or a newer Lua version:
You can use `table.new` or `table.create` respectively to preallocate hash and array part to fit a certain size.
...
As we've seen above, there can be an arbitrarily bad ratio of free slots to used slots in pathological cases.

In the standard implementations of PUC Lua and LuaJIT,
table iteration needs to scan the entire array part to find all non-nil values.
This takes time proportional to the size of the table in memory,
not to the actual number of live key-value pairs in the table.

Now suppose you were to iterate the table `t = {1, nil, ..., nil}` from the above example using `pairs`:
...
In contrast, `ipairs` *is* guaranteed to be (expected) linear time in the size of a sequence,
because each table access is (expected) constant time, and `ipairs` stops on the first `nil` it encounters;
it does not need to scan the entire array part.

In a nutshell, there are two nasty consequences for asymptotics to be aware of:
Neither memory usage of a table nor the time required for its iteration
necessarily need to be proportional to the number of currently live entries.
...
<!-- TODO evaluation order; beautiful global-local example -->
