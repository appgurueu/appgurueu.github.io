...
In modern programming languages, strings are typically *immutable*, to avoid many of the surprises that
...
and makes string comparisons cheap constant time pointer comparisons.

This is especially important in languages like Lua which use strings *a lot* as hash map (table) keys:
Indexing tables needs to be very efficient to make using tables as "structs" viable. [^lua]
...
(With string interning, this is also rather hard to avoid, as the full result needs to be hashed.)
...
[^strs]: There is another exception, another operation on strings:
Lexicographic comparison of strings using `<`, `>`, `<=` or `>=`, takes linear time again.
...
This is typically an "array list" (in Java terms) of characters which is grown using powers of two,
making the "append character" operation amortized constant time (vs. linear time for string concatenation).

In Lua and many other scripting languages, a list (table) of strings to be concatenated serves as a string builder [^luajit]:
...
Some languages take another approach by just having mutable strings.
Ruby has both a copying `+` operator, which produces a new string that is the concatenation of the two arguments,
and a `dst << src` operator, which *mutates* `dst` by appending `src`. [^cpp]
In such languages, strings are effectively array lists of characters,
directly making them suitable for use as string builders.

[^cpp]: In other languages, this operator is known as `+=`.
C++ has both "`ostringstream`s", which are effectively string builders,
and mutable strings that themselves have a capacity and can be appended to.

[^luajit]: Using a table of strings as a string buffer has some caveats, e.g. when it comes to data density.
LuaJIT also offers a dedicated, more efficient [string buffer extension](https://luajit.org/ext_buffer.html).
...
For example, you can cheat at benchmarks by comparing a solution which eagerly pays the cost of string concatenation
against one which doesn't (but which will most likely have to pay them sometime later).
...
This runs efficiently in linear time on any serious JS engine (V8, SpiderMonkey, JavaScriptCore),
...
Only when we ask for the length of the complete string is this data structure traversed and turned into a plain character array.
...
for (let i = 0; i < 1e7; ++i) {
	s += "a";
	if (s[i] != "a") throw "what"; // harmless?
}
console.log(s.length);
```

On my machine this takes something like 8 hours (on V8).
...
If done right, using a string builder should result in simultaneously more efficient and more readable code.
Often code can be elegantly rewritten to a more "functional" style using widely available standard library helpers
that concatenate a list / iterator of strings (e.g. `table.concat`, `str.join`, `Array.prototype.concat`, and so on).
...
If you write recursive serialization code, you should usually pass around a string builder instead of returning strings.
